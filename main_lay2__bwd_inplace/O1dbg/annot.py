...
'''
    ...
    algebra_conj_lines, range_conj_lines = bound_vecreg([2295, 3750, 3750, 13800, 20600, 20600, 13800, 20600, 20600],
...
def annot_k0_iter(annotator, i, k0):
    ...
    fa_ghost_property = ConjunctionLines([
        [f'%fa{j}_{i}{k0} = {make_vector(arr_in[(9 - j) % 9 + 9 * (k0 + 2 * i)])}'] for j in range(9)
    ])
    ...
    algebra_conj_lines, range_conj_lines = bound_array(8795, [arr_mem[(i * 2 + k0) * 9 + j] for j in range(9)])
...
def annot_i_iter(annotator, i):
    k0_iter_0_end = annotator.find_first_line('PC = 0x555555153c')
    k0_iter_1_end = annotator.find_first_line('PC = 0x55555515e0')
...
def annot(annotator):
    ...
    i_loop_begin = annotator.find_first_line('PC = 0x55555514a0')
    i_loop_end = annotator.find_first_line('PC = 0x5555551600', offset=-2)

    i_iter_ends = []
    i_iter_end = i_loop_begin
    while True:
        i_iter_end = annotator.find_first_line('PC = 0x55555515e0', i_iter_end)
        if i_iter_end == -1:
            break
        i_iter_end += 2
...
annotator = AnnotatorState('./main_lay2__bwd_inplace/O1dbg_neon_raw.cl', )
output_lines = annot(annotator)
with open('./main_lay2__bwd_inplace/O1dbg/O1dbg_neon.cl', 'w') as f:
