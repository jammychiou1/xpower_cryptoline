...
''')
    ...
    print(f'cut (* {cut_id} *)')
    for j in range(9):
        print(f'    {memory_arr(0x7ffffff080 + 16 * j, 8, 2)} =')
    ...
    {memory_arr(0x7ffffff080, 8, 2)} <= {format_coefs([6885] * 8)} /\\
    {memory_arr(0x7ffffff080, 8, 2)} >= {format_coefs([-6885] * 8)} /\\

    {memory_arr(0x7ffffff090, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7ffffff090, 8, 2)} >= {format_coefs([-8340] * 8)} /\\

    {memory_arr(0x7ffffff0a0, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7ffffff0a0, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7ffffff0b0, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7ffffff0b0, 8, 2)} >= {format_coefs([-8340] * 8)} /\\

    {memory_arr(0x7ffffff0c0, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7ffffff0c0, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7ffffff0d0, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7ffffff0d0, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7ffffff0e0, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7ffffff0e0, 8, 2)} >= {format_coefs([-8340] * 8)} /\\

    {memory_arr(0x7ffffff0f0, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7ffffff0f0, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7ffffff100, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7ffffff100, 8, 2)} >= {format_coefs([-8340] * 8)} /\\
...
def annot_k0_iter_0(k0_iter_0, i, prologue_cut_id):
    ...
    ntt9_2x_end = find_first_line(k0_iter_0, 'PC = 0x5555551944', ntt9_2x_begin)
...
ghost %fa0_{i}0@sint16[8], %fa1_{i}0@sint16[8], %fa2_{i}0@sint16[8], %fa3_{i}0@sint16[8], %fa4_{i}0@sint16[8], %fa5_{i}0@sint16[8], %fa6_{i}0@sint16[8], %fa7_{i}0@sint16[8], %fa8_{i}0@sint16[8] :
    ...
    %fa8_{i}0 = [L0x7fffffeff0, L0x7fffffeff2, L0x7fffffeff4, L0x7fffffeff6, L0x7fffffeff8, L0x7fffffeffa, L0x7fffffeffc, L0x7fffffeffe]
  &&
    %fa0_{i}0 = %v0 /\\ %fa1_{i}0 = %v1 /\\ %fa2_{i}0 = %v2 /\\ %fa3_{i}0 = %v3 /\\
    %fa4_{i}0 = %v4 /\\ %fa5_{i}0 = %v5 /\\ %fa6_{i}0 = %v6 /\\ %fa7_{i}0 = %v7 /\\
    %fa8_{i}0 = [L0x7fffffeff0, L0x7fffffeff2, L0x7fffffeff4, L0x7fffffeff6, L0x7fffffeff8, L0x7fffffeffa, L0x7fffffeffc, L0x7fffffeffe];
...
''')
    for j in range(9):
        rhs = [f'arr{i}0{(9 - j) % 9}{k}' for k in range(8)]
...
def annot_k0_iter_1(k0_iter_1, i, prologue_cut_id):
    ...
    ntt9_2x_end = find_first_line(k0_iter_1, 'PC = 0x55555519dc', ntt9_2x_begin)
...
ghost %fa0_{i}1@sint16[8], %fa1_{i}1@sint16[8], %fa2_{i}1@sint16[8], %fa3_{i}1@sint16[8], %fa4_{i}1@sint16[8], %fa5_{i}1@sint16[8], %fa6_{i}1@sint16[8], %fa7_{i}1@sint16[8], %fa8_{i}1@sint16[8] :
    ...
    %fa8_{i}1 = [L0x7fffffeff0, L0x7fffffeff2, L0x7fffffeff4, L0x7fffffeff6, L0x7fffffeff8, L0x7fffffeffa, L0x7fffffeffc, L0x7fffffeffe]
  &&
    %fa0_{i}1 = %v0 /\\ %fa1_{i}1 = %v1 /\\ %fa2_{i}1 = %v2 /\\ %fa3_{i}1 = %v3 /\\
    %fa4_{i}1 = %v4 /\\ %fa5_{i}1 = %v5 /\\ %fa6_{i}1 = %v6 /\\ %fa7_{i}1 = %v7 /\\
    %fa8_{i}1 = [L0x7fffffeff0, L0x7fffffeff2, L0x7fffffeff4, L0x7fffffeff6, L0x7fffffeff8, L0x7fffffeffa, L0x7fffffeffc, L0x7fffffeffe];
...
''')
    for j in range(9):
        rhs = [f'arr{i}1{(9 - j) % 9}{k}' for k in range(8)]
...
def annot_i_iter(i_iter, i, prologue_cut_id):
    ...
    k0_iter_0_end = find_first_line(i_iter, 'PC = 0x555555198c')
    k0_iter_1_end = find_first_line(i_iter, 'PC = 0x5555551a30', k0_iter_0_end)
...
def annot(lines):
    ...
    i_loop_begin = find_first_line(lines, 'PC = 0x55555518f0')
    i_loop_end = find_first_line(lines, 'SP = 0x7ffffff110', i_loop_begin)

    i_iter_ends = []
    i_iter_end = i_loop_begin
    while True:
        i_iter_end = find_first_line(lines, 'PC = 0x5555551a30', i_iter_end)
        if i_iter_end == -1:
            break
        i_iter_end += 2
    ...
    for i in range(10):
        for k0 in range(2):
            for j in range(9):
                ...
                print(f'    {format_arr(arr)} <= {format_arr(["4585"] * 8)} /\\')
    print()
    for i in range(10):
        for k0 in range(2):
            for j in range(9):
    ...
    for i in range(10):
        for k0 in range(2):
            for j in range(9):
                arr = [f'arr{i}{k0}{j}{k}' for k in range(8)]
                print(f'    {format_arr(arr)} <=s {format_arr(["4585@16"] * 8)} /\\')
    print()
    for i in range(10):
        for k0 in range(2):
            for j in range(9):
    ...
    for i in range(10):
        for k0 in range(2):
            for j in range(9):
                arr = [f'arr{i}{k0}{j}{k}' for k in range(8)]
                print(f'mov {memory_arr(0x55555728f8 + 16 * (j + 9 * (k0 + 2 * i)), 8, 2)} {format_arr(arr)};')
    print()
    ...
    for i in range(10):
        for k0 in range(2):
            for j in range(9):
                arr = [f'arr{i}{k0}{j}{k}' for k in range(8)]
                print(f'    {memory_arr(0x55555728f8 + 16 * (j + 9 * (k0 + 2 * i)), 8, 2)} = {format_arr(arr)} /\\')
    print()
