...
''')
    ...
    {memory_arr(0x7fffffc2a0, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7fffffc2a0, 8, 2)} >= {format_coefs([-8340] * 8)} /\\

    {memory_arr(0x7fffffc2b0, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7fffffc2b0, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7fffffc2c0, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7fffffc2c0, 8, 2)} >= {format_coefs([-8340] * 8)} /\\

    {memory_arr(0x7fffffc2d0, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7fffffc2d0, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7fffffc2e0, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7fffffc2e0, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7fffffc2f0, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7fffffc2f0, 8, 2)} >= {format_coefs([-8340] * 8)} /\\

    {memory_arr(0x7fffffc300, 8, 2)} <= {format_coefs([8795] * 8)} /\\
    {memory_arr(0x7fffffc300, 8, 2)} >= {format_coefs([-8795] * 8)} /\\

    {memory_arr(0x7fffffc310, 8, 2)} <= {format_coefs([8340] * 8)} /\\
    {memory_arr(0x7fffffc310, 8, 2)} >= {format_coefs([-8340] * 8)} /\\
...
def annot_k0_iter_0(k0_iter_0, i, prologue_cut_id):
    ...
    ntt9_2x_end = find_first_line(k0_iter_0, 'PC = 0x55555514f4', ntt9_2x_begin)
...
def annot_k0_iter_1(k0_iter_1, i, prologue_cut_id):
    ...
    ntt9_2x_end = find_first_line(k0_iter_1, 'PC = 0x555555158c', ntt9_2x_begin)
...
def annot_i_iter(i_iter, i, prologue_cut_id):
    ...
    k0_iter_0_end = find_first_line(i_iter, 'PC = 0x555555153c')
    k0_iter_1_end = find_first_line(i_iter, 'PC = 0x55555515e0', k0_iter_0_end)
...
def annot(lines):
    ...
    i_loop_begin = find_first_line(lines, 'PC = 0x55555514a0')
    i_loop_end = find_first_line(lines, 'SP = 0x7fffffc320', i_loop_begin)

    i_iter_ends = []
    i_iter_end = i_loop_begin
    while True:
        i_iter_end = find_first_line(lines, 'PC = 0x55555515e0', i_iter_end)
        if i_iter_end == -1:
            break
        i_iter_end += 2
